#!/bin/bash
# ========================================================================
# StormDB Cross-Platform Testing Script
# Tests on both ARM64 (native) and x86_64 (emulated) platforms
# ========================================================================
...
test_x86_64_linux() {
    ...
    local base_image=""
    local install_deps=""
    if docker image inspect stormdb-base:latest >/dev/null 2>&1; then
        echo -e "${GREEN}[CACHE]${NC} Using cached base image stormdb-base:latest"
        base_image="stormdb-base:latest"
        install_deps="false"
    elif docker image inspect stormdb-builder-ubuntu:latest >/dev/null 2>&1; then
        echo -e "${GREEN}[CACHE]${NC} Using existing stormdb-builder-ubuntu:latest"
        base_image="stormdb-builder-ubuntu:latest"
        install_deps="true"
    else
        echo -e "${YELLOW}[BUILD]${NC} Building cached base image..."
...
# Function to test on ARM64 Linux (if available)
test_arm64_linux() {
    log_info "Testing on ARM64 Linux (native platform)..."
    
    echo "🐧 ARM64 Linux Native Build"
    echo "============================"
    
    # Try to use ARM64 Ubuntu image
    if docker run --rm \
        ...
        ubuntu:22.04 \
        bash -c "
            echo 'Setting up ARM64 Linux environment...'
            apt-get update -qq && apt-get install -y -qq \
                build-essential gcc make libyaml-dev libpq-dev pkg-config
            
            echo 'Building on ARM64 Linux...'
            make clean
            make info
            make debug
            
            echo 'Testing ARM64 Linux build...'
            if [[ -f 'bin/stormdb-debug' ]]; then
                echo '✅ ARM64 Linux build successful'
                ./bin/stormdb-debug --version
                
                echo 'Binary architecture:'
                file bin/stormdb-debug
                
                echo '✅ ARM64 Linux testing completed'
            else
                echo '❌ ARM64 Linux build failed'
                exit 1
            fi
        " 2>/dev/null; then
        log_success "ARM64 Linux testing successful"
    else
        log_warning "ARM64 Linux testing skipped (image pull failed or not available)"
    fi
}
...
main() {
    ...
    case "${1:-all}" in
        ...
        "help"|"--help"|"-h")
            ...
            echo "  arm64-linux - Test on ARM64 Linux (native)"
